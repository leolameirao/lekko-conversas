...
const getBodyMessage = (msg) => {
    ...
    try {
        ...
        if (!objKey) {
            logger_1.logger.warn(`#### Nao achou o type em getBodyMessage: ${type}
...
const downloadMedia = async (msg) => {
    ...
    while (contDownload < 10 && !stream) {
        try {
            contDownload > 1
                ? logger_1.logger.warn(`Tentativa ${contDownload} de baixar o arquivo`)
                : "";
            ...
            logger_1.logger.warn(`>>>> erro ${contDownload} de baixar o arquivo`);
        }
    }
...
const handleMessage = async (msg, wbot, companyId) => {
    ...
    try {
        ...
        if (unreadMessages === 0 && whatsapp.complationMessage && (0, Mustache_1.default)(whatsapp.complationMessage, contact) === bodyMessage) {
            return;
        }
