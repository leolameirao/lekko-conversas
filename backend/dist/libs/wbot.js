    ...
    return new Promise(async (resolve, reject) => {
        try {
            (async () => {
                ...
                const { state, saveCreds } = await (0, baileys_1.useMultiFileAuthState)(folderSessions);
                // const { state, saveCreds } = await useMultiFileAuthState(whatsapp); // em memoria
                // const { state, saveState } = await authState(whatsapp); // antes do update jhonny
                wsocket = (0, baileys_1.default)({
                    logger: loggerBaileys,
                    printQRInTerminal: false,
                    // auth: {
                    //   creds: state.creds,
                    //   keys:  (state.keys, logger),
                    // },
                    auth: state,
                    version: [2, 2323, 4],
